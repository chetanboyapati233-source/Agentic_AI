...
st.set_page_config(page_title="Company AI Assistant (LangChain)", page_icon="🦜", layout="wide")

st.title("🦜 Company AI Assistant — Built with LangChain")
st.caption("Same app as before — now powered by LangChain AgentExecutor + LangChain tools")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**LangChain vs Manual:**")
    st.markdown("- Tools defined with `@tool` decorator")
    st.markdown("- Agent loop handled by `AgentExecutor`")
    st.markdown("- No manual JSON tool schema needed")
    st.markdown("- `intermediate_steps` shows agent reasoning")
...
if question := st.chat_input("Ask a question about employees, sales, or company policy..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("LangChain agent thinking..."):
